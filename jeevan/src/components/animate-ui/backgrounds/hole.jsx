...
function HoleBackground({
  ...
  const tweenValue = React.useCallback((start, end, p, ease = null) => {
    const delta = end - start;
    const easeFn = ease === 'inExpo' ? easeInExpo : linear;
    return start + delta * easeFn(p);
  }, []);

  const tweenDisc = React.useCallback((disc) => {
  ...
  const initParticle = React.useCallback((start = false) => {
    const sx =
  ...
  const drawDiscs = React.useCallback((ctx) => {
    ctx.strokeStyle = strokeColor;
    ctx.lineWidth = 2;
  ...
  const drawParticles = React.useCallback((ctx) => {
    ctx.save();
    ctx.clip(stateRef.current.clip.path);
